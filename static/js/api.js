// api.js v3.0 — ESPN + nba_api + Polymarket
...
async function fetchESPNScoreboard() {
  ...
  try {
    ...
    events.forEach(event => {
      ...
      const match = gameData.find(g => g.home === homeAbbr || g.away === awayAbbr);
      if (match) {
...
async function fetchESPNInjuries() {
  ...
  try {
    ...
    const live = raw.map(p => ({
      ...
      _prev:  injuries.find(i => i.name.includes((p.name || '').split(' ').pop()))?.status || null,
    })).slice(0, 12);
...
async function fetchNBATeamStats() {
  ...
  try {
    const data = await apiFetch(API.nbaTeamStats(), {}, 15000);
    if (data.error) throw new Error(data.error);
    const apiTeams = data.teams || [];
    apiTeams.forEach(at => {
      const t = teams.find(t => t.abbr === at.abbr);
      if (t) {
        t.ortg = at.ortg; t.drtg = at.drtg;
        t.pace = at.pace; t._live = true;
        t._wins = at.wins; t._losses = at.losses;
      }
    });
    gameData.forEach(g => {
    ...
    renderEngineDetail();
    setBadge('effDb', 'live');
    setStatus('NBAApi', 'live', `nba_api · ${apiTeams.length} teams`);
  } catch (e) {
...
async function fetchMatchupFeatures(homeAbbr, awayAbbr) {
  try {
    const data = await apiFetch(`/api/nba/features?home=${homeAbbr}&away=${awayAbbr}`, {}, 12000);
...
async function fetchPolymarket() {
  ...
  try {
    ...
    liveMarkets = enriched.map(r => r.value || r.reason).filter(Boolean)
      .map(m => {
...
console.log('✅ api.js v3.0 loaded');
